...
<?php if ($_SESSION["tip_rep"] == 31){?>
	   <a href='tipo_rep_3.php?menu=7'>Salir</a>
 <?php } if ($_SESSION["tip_rep"] == 32) { ?>
 	   <a href='con_factura.php?menu=11'>Salir</a>
<?php } ?>
...
<font size="+1"  style="" >
<?php
	if ($_SESSION["tip_rep"] == 31) {	
    echo encadenar(4)."VENTAS BANCARIZACION ".encadenar(10). "PERIODO FISCAL "
	.encadenar(2).$mes."/".$anio;
	}
	if ($_SESSION["tip_rep"] == 24) {	
    echo encadenar(4)."VENTAS BANCARIZACION ".encadenar(2).$f_des  ."AL "
	.encadenar(2).$f_has;
	}
?>
<br>
...
<?php 
  //Datos del cart_det_tran	
   // echo $f_des2.encadenar(2).$f_has2;
	...
	$t_debito = 0; 
	if ($_SESSION["tip_rep"] == 31) {				
	$con_tpa = "Select * From fac_bancari where month(FAC_BAN_FECHA) = '$mes' and year(FAC_BAN_FECHA) = '$anio' and FAC_BAN_TIPO = 2
	            order by FAC_BAN_FECHA,FAC_BAN_NRO";
	}
	if ($_SESSION["tip_rep"] == 32) {				
	$con_tpa = "Select * From fac_bancari where FAC_BAN_FECHA between '$f_des2' and '$f_has2' and FAC_BAN_TIPO = 2
	            order by FAC_BAN_FECHA,FAC_BAN_NRO";
	}			
