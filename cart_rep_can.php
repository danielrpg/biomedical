...
$fec_pro = $_POST['fec_des'] ; 
$fec_has = $_POST['fec_has'] ; 
$mes = substr($fec_pro,3,2);
...
<?php
echo encadenar(45)."Detalle de Cartera Cancelados del ".encadenar(3).$fec_pro.encadenar(3)."al".encadenar(3).$fec_has;
?>
</font>
<br><br>
<?php
...
$con_car  = "Select * From cart_maestro, cart_deudor, cliente_general
             where (CART_ESTADO between $est1 and $est2) and (CART_FCH_CAN between '$f_pro' and '$f_has')
			  and CART_DEU_NCRED = CART_NRO_CRED
...
$nro = 0;
 ...
 if ($c_grup > 0){
	...
	$con_tde = "Select * From cart_det_tran where CART_DTRA_NCRE = $cod_cre and CART_DTRA_TIP_TRAN = 1 
	            AND CART_DTRA_FECHA <= '$f_has' and CART_DTRA_CCON = 131 
	...
	$con_tpa = "Select * From cart_det_tran where CART_DTRA_NCRE = $cod_cre and CART_DTRA_TIP_TRAN = 2 
	  AND CART_DTRA_FECHA <= '$f_has' and  ((CART_DTRA_CCON BETWEEN 131 AND 134) or (CART_DTRA_CCON = 531)) 
	  and CART_DTRA_USR_BAJA is null";
