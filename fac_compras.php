...
<?php if($_GET["menu"]==1){?>
	    <a href='tipo_rep_3.php?menu=9'>Salir</a>
<?php } elseif($_GET["menu"]==2){?>
		<a href='con_factura.php?menu=2'>Salir</a>
<?php } ?>
...
<font size="+1"  style="" >
<?php
	if ($_SESSION["tip_rep"] == 26) {	
    echo encadenar(4)."LIBRO DE COMPRAS IVA".encadenar(10). "PERIODO FISCAL "
	.encadenar(2).$mes."/".$anio;
	}
	if ($_SESSION["tip_rep"] == 24) {	
    echo encadenar(4)."LIBRO DE COMPRAS IVA DE ".encadenar(2).$f_des  ."AL "
	.encadenar(2).$f_has;
	}
?>
<br>
...
<font size="0"  style="" >
    ...
    <tr>
	    ...
	    <th align="center"><font size="-1">RAZON SOCIAL</th> 
	   	<th align="center"><font size="-1">DE RAZON SOCIAL</th>  
...
<?php 
  //Datos del cart_det_tran	
   // echo $f_des2.encadenar(2).$f_has2;
	...
	$t_neto = 0;
	$t_debito = 0; 
	if ($_SESSION["tip_rep"] == 26) // cambir a 26 pra que sea compra y cambiar la consulta************
	 {				
	$con_tpa = "Select * From factura where month(FACTURA_FECHA) = '$mes' and year(FACTURA_FECHA) = '$anio' and FACTURA_TIPO = 1 order by FACTURA_FECHA,FACTURA_NUMERICO ";
	}
	if ($_SESSION["tip_rep"] == 28) {				
	$con_tpa = "Select * From factura where FACTURA_FECHA between '$f_des2' and '$f_has2' and FACTURA_TIPO = 1
	...
	<tr>
		...
		<td align="left" ><?php echo $nombre; ?></td>
	   	<td align="right" ><?php echo number_format($factura, 0, '.',''); ?></td>
