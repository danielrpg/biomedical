...
function Cliente(){
	...
	this.enviarFormularioNuevoCliente = function (){
		...
		$('#form_vent_nuevo_cliente').submit(function(evt){
			...
			}else if($.trim($('#txt_vent_cliente_fecha_nac').val()) == ""){
			     util.mostrarMensajeAlerta(0, 'La Fecha de Nacimeinto no puede estar vacio');  	
	 ...
	 */
	 this.editarCliente = function(id_cliente){
	 	...
	 	$.getJSON('?action=clientes&tp=getDataCliente', {client_id:id_cliente}, function(res){
	 		...
	 		$('#txt_vent_cliente_fecha_nac_edit').val(new Cliente().cambiarFormatoFechaNormal(res.vent_cli_fch_nac));
