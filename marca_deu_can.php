...
if ($cod_mon == 1){
 ...
 //$res_fca = mysql_query($con_fca)or die('No pudo seleccionarse fond_can');
  // while ($lin_fca = mysql_fetch_array($res_fca)) {
   //       $cta_fdo = $lin_fca['cuenta'];
$con_fon  = "Select * From cart_maestro where CART_ESTADO = 109";
              
$res_fon = mysql_query($con_fon);
$nro = 0;
   while ($lin_fon = mysql_fetch_array($res_fon)) {
         $cod_cre = $lin_fon['CART_NRO_CRED']; 
		 echo $cod_cre;
	     $act_maestro = "update cart_deudor set CART_DEU_USR_ALTA = 'va' 
