...
<title>Detalle de Transaccion Banco a revertir</title>
<meta charset="UTF-8">
...
<!--Cabecera del modulo con su alerta-->
                      <h2> <img src="img/documents_64x64.png" border="0" alt="Modulo" align="absmiddle">DETALLE BANCOS</h2>
...
<?php 
  //Datos del cart_det_tran	
   // echo $f_des2.encadenar(2).$f_has2;
	...
	//echo $tipo." ".$tran;
	$con_tpa = "Select *
	            From caja_deposito where
	            (CAJA_DEP_FECHA between '$f_des' and '$f_des') and
				 CAJA_DEP_TIPO = $tipo and
				 CAJA_DEP_NRO = $tran  and
	             CAJA_DEP_USR_BAJA is null
    ...
    $res_tpa = mysql_query($con_tpa);
	    ...
	    while ($lin_tpa = mysql_fetch_array($res_tpa)) { // 1a
			...
			$cta_ctb =  $lin_tpa['CAJA_DEP_CTA_CTB'];
			$descri =  $lin_tpa['CAJA_DEP_QUIEN'];
			$deb_hab = $lin_tpa['CAJA_DEP_DEB_HAB'];
...
<center>
   ...
   <input class="btn_form" type="submit" name="accion" value="Revertir Banco">
