...
<?php
//if ($mone == 1){
    echo encadenar(35)."LISTADO DE RECIBOS UTILIZADOS ".encadenar(2);
//  }
// if ($mone == 2){
...
<font size="+1"  style="" >
<?php
echo encadenar(40)."DEL".encadenar(3).$f_des.encadenar(3)."AL".encadenar(3).$f_has;
?>
</font>
<br><br>
<font size="0"  style="" >
	...
	<tr>
	    ...
	    <th align="center"><font size="-1">NRO. TRAN. ORIGEN</th> 
		<th align="center"><font size="-1">DESCRIPCION</th>
		<th align="center"><font size="-1">MONTO TOTAL</th>
...
<?php 
 ...
 //ECHO "AQUIIII".$f_has2;
		...
		$con_reci = "Select * From recibo_deta where  REC_DET_FECHA >= '$f_des2' and REC_DET_FECHA  <='$f_has2'
		             and REC_DET_USR_BAJA is null order by REC_DET_NRO";
            $res_reci = mysql_query($con_reci);
	                    while ($lin_reci = mysql_fetch_array($res_reci)) {
							  ...
							  $nom_ases = $lin_reci['REC_DET_USR_ALTA'];
 //echo  $nro_tran. "  ". $nro_rec;

$p_tot = 0;
	$con_tpa = "select * FROM cjach_transac
                WHERE CJCH_TRAN_FECHA ='$fec_rec' and CJCH_TRAN_NRO_COR= $nro_caja and CJCH_TRAN_TIPO_OPE = 2 and  substr(CJCH_TRAN_CTA_CONT,1,4)='1111'
and CJCH_TRAN_USR_BAJA is null ";
    $res_tpa = mysql_query($con_tpa) ;
	    ...
	    while ($lin_tpa = mysql_fetch_array($res_tpa)) { // 1a
			...
			$desc = $lin_tpa['CJCH_TRAN_DESCRIP'];
			$p_tot = $lin_tpa['CJCH_TRAN_IMPORTE'] * -1;
