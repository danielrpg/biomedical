...
<br><br><br>
 <?php
		 ...
		 if (isset($_SESSION['cta_iue'])){
			$cta_iue = $_SESSION['cta_iue'];
			} else {
				 $cta_iue = 0;
				}
			 if (isset($_SESSION['cta_it'])){	
            $cta_it = $_SESSION['cta_it'];
			} else {
				 $cta_it = 0;
				}
			$nro_asig= $_SESSION['nro_asig'];
			if (isset($_SESSION['monto_fis'])){
                $monto_fis = $_SESSION['monto_fis'];
				} else {
				 $monto_fis = 0;
				}
			if (isset($_SESSION['monto_125'])){	
            $monto_125 = $_SESSION['monto_125'];
			    } else {
				 $monto_125 = $_SESSION['monto_125'];
				}
						
			if (isset($_SESSION['monto_3'])){
			    $monto_3 = $_SESSION['monto_3'];
				  } else {
				 $monto_3 = $_SESSION['monto_3'];
				}
			$cta_ctbg = $_SESSION['cta_ctbg'];
			echo "xxxxx".$_SESSION['monto_t']."yyy".$_SESSION['egre_bs_sus']."zzzz".$_SESSION['monto_eq'];
...
//if ($_SESSION['detalle'] == 3){
 ...
 //  echo $_SESSION['monto_eq'];
	...
	$imp_or = $_SESSION['monto_t'];		 
   	$impo_sus = $_SESSION['monto_t']/$tc_ctb;
...
$cod_cja = $_SESSION['cod_cja'];
 echo "Aquiiiii"; //$nro_tr_ingegr.encadenar(2).$nro_tr_caj.encadenar(2).$c_agen.encadenar(2).$deb_hab.encadenar(2).$cta_ctb.encadenar(2).
		// $fec1.encadenar(2).$descrip.encadenar(2).$imp_or.encadenar(2).$imp_or.encadenar(2).$log_usr;
 
 
 
 //Graba cuenta Caja Chica
...
if(isset($_SESSION['t_fac_fis'])){
 ...
 // if ($_SESSION['egre_bs_sus'] == 2){
  ...
  echo $_SESSION['t_fac_fis']."***".$imp_or."&&&".$cta_ctbg;
  //  $impo_sus = $_SESSION['monto_eq'];
...
$consulta = "insert into cjach_transac(CJCH_TRAN_NRO_CJA,
                                       ...
                                       CJCH_TRAN_FCH_HR_BAJA
												 ...
												 '$log_usr',
												  ...
												  null,
												  '0000-00-00 00:00:00')";
$resultado = mysql_query($consulta)or die('No pudo insertar caja_ing_egre  yy: ' . mysql_error()); 
