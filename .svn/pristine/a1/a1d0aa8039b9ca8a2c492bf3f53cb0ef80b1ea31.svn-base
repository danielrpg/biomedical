...
function ImprovedTable($header,$saldo,$saldo_sus)
{
  ...
  //  $this->Cell(80);
	  ...
	  $this->SetFont('Arial','',8);
	    $txt2 = 'Intervienen en la celebración del presente contrato:';
		 $this->MultiCell(0,5,$txt1.' '.$txt2);
		   ...
		   if ($_SESSION['EMPRESA_TIPO'] == 1){
		   $txt = 'a).- '.' PRO-DESARROLLO MICROEMPRESARIAL SRL.- PRODEMIC S.R.L., empresa legalmente constituida y representada por el   '.$emp_ger.' con '.$emp_cig.'., con domicilio en '.$emp_dir. ' de la '. $donde.', quien en adelante y para fines del presente contrato se denominara “EL ACREEDOR". ';
		  }
   ...
   while ($lin_sol = mysql_fetch_array($res_sol)) {
	...
	//Deudores	  
	$consulta  = "Select CRED_DEU_INTERNO, CLIENTE_COD_ID,CLIENTE_AP_PATERNO, CLIENTE_AP_MATERNO, CLIENTE_NOMBRES,CLIENTE_EST_CIVIL,
	CLIENTE_AP_ESPOSO, CRED_DEU_RELACION, CLIENTE_DIRECCION, CLIENTE_ZONA
	 From cliente_general, cred_deudor where CRED_SOL_CODIGO = $nro_sol 
...
if ($ap_espo > 0){
	...
	/*  $txt = 'NOVENA.- (DOMICILIO ESPECIAL).- Para el caso de cobro judicial o para cualquier efecto legal emergente del presente contrato, "LOS DEUDORES", con la facultad que les otorga el parágrafo II del Art. 29 del Código  Civil, señalan como domicilio especial los declarados en las cláusula PRIMERA. En dichos domicilios se practicaran valida y legalmente todas las citaciones, notificaciones, avisos y comunicaciones, sin lugar a posterior observación, incidente o recurso alguno.';
	  $this->MultiCell(0,5,$txt); */
	  $txt = 'NOVENA.- (FACULTAD DE EL ACREEDOR).- "LOS DEUDORES" autorizan a "EL ACREEDOR" a ceder, subrogar y/o transmitir a cualquier titulo el presente contrato, total o parcial, a favor de terceros, sin necesidad de aceptación de ninguno de  ellos y mucho menos su intervención en los contratos que suscriba "EL ACREEDOR" con terceros, siendo suficiente la información que proporcione el "EL ACREEDOR" a su elección, antes o después de la cesión, subrogación y/o transferencia.';
	   $this->MultiCell(0,5,$txt); 
