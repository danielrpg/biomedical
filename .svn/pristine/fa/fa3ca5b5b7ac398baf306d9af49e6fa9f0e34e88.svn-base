...
echo "Fecha de Calculo   ".encadenar(2).$fec_c;
?>
...
?>
<br>
 <form name="form1" method="post" action="cobro_retro_op.php" style="border:groove" target="_self"  >
  <?php echo "APLICACION  DE  PAGO";?>
	<table width="50%"  border="1" cellspacing="1" cellpadding="1" align="center">
	  <tr>
           <th width="22%" bgcolor="#66CCFF" scope="col"><border="0" alt="" align="absmiddle" />CREDITO</th>
	...
	while ($lin_cobt = mysql_fetch_array($res_cobt)){
		 ...
		 $mon_pcap = montos_recupera_dos($ncre,$f_cal,1);
	     $mon_pint = montos_recupera_dos($ncre,$f_cal,5); 
		// echo $d_cap."d_cap".$d_int."d_int";
		// echo $mon_pcap."mon_pcap".$mon_pint."mon_pint";
		 $deu_cap = $d_cap - $mon_pcap;
		 $deu_int = $d_int - $mon_pint;
		 $t_deuda = ($d_cap + $d_int)-($mon_pcap + $mon_pint);
		 //echo $t_deuda."t_deuda";
		 if ($monto_p >= $deu_int){
		    $a_int = $deu_int;
			$monto_p = $monto_p - $deu_int;
		    }else{
			$a_int = 0;
