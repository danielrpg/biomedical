...
<body>
	...
	<?php
       ...
       <a href="javascript:print();">Imprimir</a>
	    <a href='cjach_reportes.php?menu=8'>Salir</a>
  </div>
  <?php  
  //include("header_2.php");
...
//if ($_SESSION['continuar'] == 1){
	...
	<strong> 
	<?php
    echo encadenar(25)."Detalle de Transacciones Revertidas".encadenar(2);
	?>
	<br>
	<?php
...
<font size="0"  style="" >
  <table width="80%"  border="1" cellspacing="1" cellpadding="1" align="center">
    <tr>
	  <th  width="10%" scope="col"><border="0" alt="" align="absmiddle" />FECHA</th>
	  <th width="10%" scope="col"><border="0" alt="" align="absmiddle" />NRO. TRAN.</th>
	  <th width="30%" scope="col"><border="0" alt="" align="absmiddle" />DESCRIPCION</th>
	  <th width="10%" scope="col"><border="0" alt="" align="absmiddle" />NRO.FACT.</th>
...
if ($_SESSION['detalle'] == 1){
    $con_cart  = "select  * from cjach_transac 
	              where (CJCH_TRAN_FECHA between '$fec1' and '$fec2') and 
				  substring(CJCH_TRAN_CTA_CONT,1,6)='111102' and
				  CJCH_TRAN_USR_BAJA is not null
	              order by CJCH_TRAN_FECHA";
	...
	while ($lin_cart = mysql_fetch_array($res_cart)) {
	    //    $monto2 = $lin_cart['sum(CART_DTRA_IMPO)'];
			...
			$nro_tran = $lin_cart['CJCH_TRAN_NRO_COR'];
			$desc = utf8_encode($lin_cart['CJCH_TRAN_DESCRIP']);
			$fec = $lin_cart['CJCH_TRAN_FECHA'];
			$monto2  = $lin_cart['CJCH_TRAN_IMPORTE'];
			$mon_eqv = $lin_cart['CJCH_TRAN_IMP_EQUIV'];
			$nro_fac = $lin_cart['CJCH_TRAN_REL_FAC'];
			$nro_ope = "";
...
/*		 
	 */ ?> 
	 ...
	 <tr>
			  ...
			  <td align="left"><?php echo $nro_fac ; ?></th>
