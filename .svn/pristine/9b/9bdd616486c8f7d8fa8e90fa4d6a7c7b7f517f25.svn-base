  ...
  //  $this->Cell(80);
   ...
   $this->MultiCell(0,5,$txt1);
	   ...
	   $this->SetFont('Arial','',8);
	    $this->MultiCell(0,5,'');
	   $txt = 'Conste por el contenido del presente contrato de préstamo de dinero con garantia personal,  que con el solo reconocimiento de firmas y rúbricas ante autoridad competente surtirá los efectos que la ley le otorga y se suscribe de acuerdo a las siguientes cláusulas: ';
   ...
   while ($lin_sol = mysql_fetch_array($res_sol)) {
	...
	//Deudor	  
	$consulta  = "Select CRED_DEU_INTERNO, CLIENTE_COD_ID,CLIENTE_AP_PATERNO, CLIENTE_AP_MATERNO, CLIENTE_NOMBRES,CLIENTE_AP_ESPOSO, CRED_DEU_RELACION,
	 CLIENTE_DIRECCION From cliente_general, cred_deudor where CRED_SOL_CODIGO = $nro_sol and CRED_DEU_INTERNO = CLIENTE_COD and CRED_DEU_RELACION = 'C' and CRED_DEU_USR_BAJA is null and CLIENTE_USR_BAJA is null ";
...
$nro = 0;
	$txt = '1.2. El(a) señor(a):  '. $deudores.' '.', quien en lo sucesivo se denominara EL(A) DEUDOR(A).'; 	 
	  $this->MultiCell(0,5,$txt);	
//Codeudores
	$consulta  = "Select CRED_DEU_INTERNO, CLIENTE_COD_ID,CLIENTE_AP_PATERNO, CLIENTE_AP_MATERNO, CLIENTE_NOMBRES, CRED_DEU_RELACION, CLIENTE_DIRECCION From cliente_general, cred_deudor where CRED_SOL_CODIGO = $nro_sol and CRED_DEU_INTERNO = CLIENTE_COD and CRED_DEU_RELACION = 'D' and CRED_DEU_USR_BAJA is null and CLIENTE_USR_BAJA is null ";
    $resultado = mysql_query($consulta);
...
'; 	 
	...
	$txt = 'SEPTIMA.-(ACELERACIÓN DE PAGO) El incumplimiento de cualquiera de las cláusulas del presente contrato y la normativa vigente,  la disminución de la garantía otorgada en el patrimonio de “EL(LA)(LOS)(LAS) DEUDOR(A)(ES)(AS)”, el embargo, secuestro ó exista cualquier tipo de persecución judicial de los mismos, ó si la información proporcionada a “EL ACREEDOR”   por “EL(LA)(LOS)(LAS) DEUDOR(A)(ES)(AS)” sea falsa, inexacta o incorrecta y en términos generales cuando por cualquier causa y a sólo criterio y apreciación de “EL ACREEDOR”  , caigan en riesgo de insolvencia o se encuentre en peligro la recuperación total del crédito, dará lugar a que “EL ACREEDOR" inicie la acción ejecutiva u otra que viere por conveniente, destinada al cobro del total adeudado en concepto de capital, intereses convencionales, penales, honorarios de abogados y gastos emergentes de "EL PRÉSTAMO". '; 
	 ...
	 $txt = 'OCTAVA.-  (INFORMACION FINANCIERA).- “EL(LA)(LOS)(LAS) DEUDOR(A)(ES)(AS)”, autoriza(n) expresamente a “EL ACREEDOR",  a obtener y dar información respecto de sus antecedentes y situación económica y financiera, a través de ellos o de terceras personas, durante todo el tiempo que dure la relación con “EL ACREEDOR".';
	  $this->MultiCell(0,5,$txt);
	  $txt = 'NOVENA.- (DOMICILIO ESPECIAL).- Para el caso de cobro judicial o para cualquier efecto legal emergente del presente contrato, “EL(LA)(LOS)(LAS) DEUDOR(A)(ES)(AS)” con la facultad que les otorga el parágrafo II del Art. 29 del Código  Civil, señalan como domicilio especial los declarados en las cláusula PRIMERA. En dichos domicilios se practicaran valida y legalmente todas las citaciones, notificaciones, avisos y comunicaciones, sin lugar a posterior observación, incidente o recurso alguno.';
	  $this->MultiCell(0,5,$txt);
	  $txt = 'DECIMA.- (FACULTAD DE EL ACREEDOR).- “EL(LA)(LOS)(LAS) DEUDOR(A)(ES)(AS)” autorizan a "EL ACREEDOR" a ceder, subrogar y/o transmitir a cualquier titulo el presente contrato, total o parcial, a favor de terceros, sin necesidad de aceptación de ninguno de  ellos y mucho menos su intervención en los contratos que suscriba "EL ACREEDOR" con terceros, siendo suficiente la información que proporcione el "EL ACREEDOR" a su elección, antes o después de la cesión, subrogación y/o transferencia.';
	   $this->MultiCell(0,5,$txt); 
...
if ($mes == 1){
 ...
 $consulta  = "Select CRED_DEU_INTERNO, CLIENTE_COD_ID,CLIENTE_AP_PATERNO, CLIENTE_AP_MATERNO, CLIENTE_NOMBRES, CRED_DEU_RELACION, CLIENTE_DIRECCION From cliente_general, cred_deudor where CRED_SOL_CODIGO = $nro_sol and CRED_DEU_INTERNO = CLIENTE_COD and CRED_DEU_RELACION <> 'D' 
