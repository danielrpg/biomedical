  ...
  //  $this->Cell(80);
   ...
   $this->MultiCell(0,5,$txt1);
	   ...
	   $this->SetFont('Arial','',8);
	    $this->MultiCell(0,5,'');
	   $txt = 'Conste por el contenido del presente contrato de préstamo de dinero,  que con el solo reconocimiento de firmas y rúbricas ante autoridad competente surtirá los efectos que la ley le otorga y se suscribe de acuerdo a las siguientes cláusulas: ';
	  ...
	  $this->SetFont('Arial','',8);
	    $txt2 = 'Intervienen en la celebración del presente contrato:';
		 $this->MultiCell(0,5,$txt1.' '.$txt2);
		  if ($_SESSION['EMPRESA_TIPO'] == 1){
		 $txt = '1.1. '.' PRO-DESARROLLO MICROEMPRESARIAL SRL.- PRODEMIC S.R.L., empresa legalmente constituida y representada por el   '.$emp_ger.' con '.$emp_cig.'., con domicilio en '.$emp_dir. ' de la '. $donde.', quien en adelante y para fines del presente contrato se denominara “EL ACREEDOR". ';
		  }
   ...
   while ($lin_sol = mysql_fetch_array($res_sol)) {
	...
	$consulta  = "Select CRED_DEU_INTERNO, CLIENTE_COD_ID,CLIENTE_AP_PATERNO,
	               CLIENTE_AP_MATERNO,CLIENTE_NOMBRES,CLIENTE_AP_ESPOSO,CLIENTE_EST_CIVIL,
...
if ($ap_espo > 0){
     $deudores = $deudores." ". $linea['CLIENTE_NOMBRES']. " ".$linea['CLIENTE_AP_PATERNO']." ".$linea['CLIENTE_AP_MATERNO']." ".$d_civ.
	 " ".$linea['CLIENTE_AP_ESPOSO']. " con CI."." ".$linea['CLIENTE_COD_ID'].",  ".$direc; 
	...
	$txt = '1.2. El(a) señor(a):  '. $deudores.' '.', quien en lo sucesivo se denominara EL(A) DEUDOR(A).'; 	 
	  $this->MultiCell(0,5,$txt);	
//Codeudores
	$consulta  = "Select CRED_DEU_INTERNO, CLIENTE_COD_ID,CLIENTE_AP_PATERNO, CLIENTE_AP_MATERNO, CLIENTE_NOMBRES,CLIENTE_ZONA,CLIENTE_EST_CIVIL,
...
if ($ap_espo > 0){
     $deudores = $deudores." ". $linea['CLIENTE_NOMBRES']. " ".$linea['CLIENTE_AP_PATERNO']." ".$linea['CLIENTE_AP_MATERNO']." ".$d_civ.
	 " ".$linea['CLIENTE_AP_ESPOSO']. " con CI"." ".$linea['CLIENTE_COD_ID']."  ".$direc; 
	  $deudor = $deudor." ". $linea['CLIENTE_NOMBRES']. " ".$linea['CLIENTE_AP_PATERNO']." ".$linea['CLIENTE_AP_MATERNO']." ".$d_civ.
...
'; 	 
	 ...
	 $txt = 'CUARTA.- (PLAZO Y FORMA DE PAGO). "LOS DEUDORES", se obligan a pagar y rembolsar a "EL ACREEDOR" el préstamo concedido a que se refiere este contrato, en el plazo de '.$plz_m. '  ('.$plz_i.') '.$plazo .' contando a partir de la fecha de suscripción del presente documento, mediante el pago de '.$n_cta. ' ('.$l_cta. ') amortizaciones iguales y secuenciales, que se cancelarán cada '.$nro_d.' ('.$nrod_i. ')'. ' días, hasta la cancelación total de la obligación. Si cualquier pago del capital y/o intereses venciere un día no hábil y/o feriado, el pago será efectuado el día anterior hábil devengándose los intereses hasta la fecha efectiva de ese pago. “EL(LA)(LOS)(LAS) DEUDOR(A)(ES)(AS)” se encuentra(n) facultado(s) a realizar pagos anticipados parciales y/o totales durante la vigencia de este contrato. ';
	  $this->MultiCell(0,5,$txt);
...
if ($mes == 1){
 ...
 $fecha = "                                              Cochabamba , ".$dia." de ".$mes_l." de ".$anio;
//  $fecha = "                                              Cochabamba , 26  de Octubre de 2012";
 $this->SetFont('Arial','B',8);
 $this->MultiCell(0,25,$fecha);
  $this->MultiCell(0,25,'');
  
 $consulta  = "Select CRED_DEU_INTERNO, CLIENTE_COD_ID,CLIENTE_AP_PATERNO, CLIENTE_AP_MATERNO,CLIENTE_AP_ESPOSO, CLIENTE_EST_CIVIL,
...
$ap_espo = strlen($linea['CLIENTE_AP_ESPOSO']); 		  
if ($ap_espo > 0){
     $deudores =  $linea['CLIENTE_NOMBRES']. " ".$linea['CLIENTE_AP_PATERNO']." ".$linea['CLIENTE_AP_MATERNO']." ".$d_civ. " ".
	 $linea['CLIENTE_AP_ESPOSO']. " con CI"." ".$linea['CLIENTE_COD_ID']; 
	//  $deudor = $deudor." ". $linea['CLIENTE_NOMBRES']. " ".$linea['CLIENTE_AP_PATERNO']." ".$linea['CLIENTE_AP_MATERNO'].
	 // " de ".$linea['CLIENTE_AP_ESPOSO'].", ";
